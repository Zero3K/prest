...
class HardcodedSearchGenerator {
    constructor() {
        ...
        this.outputPathInl = path.join(this.sourceRoot, 'adjunct', 'desktop_util', 'search', 'hardcoded_searches.inl');
    }
...
`;
        ...
        // Also write the .inl file that the build expects
        const inlContent = `/* Generated by build/scripts/generate_hardcoded_searches.js */
// Hardcoded search engines inline file
${searchEngines.map((engine, index) => `    {"${engine.name}", "${engine.url}", "${engine.encoding}", "${engine.icon}"},`).join('\n')}
`;
        await fs.promises.writeFile(this.outputPathInl, inlContent);
        
        this.log(`Generated ${this.outputPath}`);
        this.log(`Generated ${this.outputPathInl}`);
    }
